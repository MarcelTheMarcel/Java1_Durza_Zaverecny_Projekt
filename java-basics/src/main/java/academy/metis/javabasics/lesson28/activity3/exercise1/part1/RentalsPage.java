...
public class RentalsPage {
    ...
    public void returnTitle() {
        ...
        showAllTitlesRentedBySpecificMember(filteredMap, chosenMember, false);
        System.out.println("Choose an option:");
        long rentalRecordId = inputValidator.getValidatedLong(filteredMap);
    ...
    public void showAllTitlesRentedBySpecificMember(Map<Long, RentalRecord> filteredMap, Member member, boolean isProlongation) {
        if (isProlongation) {
        ...
        for (Map.Entry<Long, RentalRecord> entry : filteredMap.entrySet()) {
            ...
            System.out.println(record.getRentalRecordId() + ". | " + titleName + " | " + authorName + " | Rented on: " +
    ...
    private long getValidatedLong(Map<Long, RentalRecord> filteredMap) {
        while (true) {
    ...
    public void prolongRental() {
        ...
        showAllTitlesRentedBySpecificMember(filteredMap, chosenMember, true);
        System.out.println("Select a rental to prolong:");
        long rentalRecordId = getValidatedLong(filteredMap);
        RentalRecord rentalRecordToProlong = filteredMap.get(rentalRecordId);

        if (rentalRecordToProlong == null) {
            System.out.println("Invalid option selected.");
            return;
        }

        if (!rentalValidator.validateMaxProlongations(rentalRecordToProlong.getTimesProlonged())) {
        ...
        if (!rentalValidator.validateIfTheTitleForProlongationIsNotInQueue(queue.getQueueRecords(), rentalRecordToProlong.getTitleId())) {
    ...
    public boolean prolongRentalRecord(RentalRecord rentalRecordToProlong) {
        rentalRecordToProlong.setReturnDate(rentalRecordToProlong.getReturnDate().plusDays(RentalRecord.MAXIMUM_RENT_DURATION_IN_DAYS));
        rentalRecordToProlong.setTimesProlonged(rentalRecordToProlong.getTimesProlonged() + 1);
        return saveRentalToTheFile(rentalRecordToProlong);
    }

    public void showRentalsPastDue() {
        System.out.println("Show rentals past due:");
        Map<Long, RentalRecord> overdueRentals = getOverDueRentals();
        if (overdueRentals.isEmpty()) {
            System.out.println("The list of past due rentals is empty...");
        } else {
            for (RentalRecord record : overdueRentals.values()) {
                System.out.println(record);
            }
        }
        System.out.println("Press enter to return to Rental page...");
    ...
    public Map<Long, RentalRecord> getOverDueRentals() {
        Map<Long, RentalRecord> overdueRentals = new HashMap<>();
        LocalDate today = LocalDate.now();
        for (Map.Entry<Long, RentalRecord> entry : rentalRecords.entrySet()) {
            RentalRecord record = entry.getValue();
            if (record.getReturnDate().isBefore(today) && !record.isReturned()) {
                overdueRentals.put(entry.getKey(), record);
            }
        }
