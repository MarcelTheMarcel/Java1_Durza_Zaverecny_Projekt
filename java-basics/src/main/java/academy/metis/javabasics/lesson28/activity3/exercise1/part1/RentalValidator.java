...
public class RentalValidator {
    ...
    public boolean isPossibleToRentTitle(long memberId, Object chosenTitle, Map<Long, RentalRecord> rentalRecords, Queue queue) {
        ...
        if (!isTheTitleAvailable(availableCopies)) {
            ...
            queue.showQueueMenu(memberId, titleId); // Returning the queue functionality
            return false;
        }
    ...
    public boolean validateIfTheTitleForProlongationIsNotInQueue(Map<Long, QueueRecord> queueRecordMap, long titleId) {
        for (QueueRecord record : queueRecordMap.values()) {
            if (record.getTitleId() == titleId && !record.isResolved()) {
                return false;
            }
        }
        return true;
