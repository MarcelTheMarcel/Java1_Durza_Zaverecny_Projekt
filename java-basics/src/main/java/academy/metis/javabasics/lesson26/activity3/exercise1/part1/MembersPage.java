...
public class MembersPage {
    ...
    public void printAllMembers() {
        ...
        if (membersMap.isEmpty()) {
            ...
            for (Member member : membersMap.values()) {
                System.out.println(index++ + ". | " + member.getFirstName() + " " + member.getSurname() + " | Date of Birth: " + member.getDateOfBirth());
            }
        }
    }
    ...
    public void handleMemberDeletionProcess() {
        if (members.isEmpty()) {
            System.out.println("No members to remove. Press enter to return to members...");
            scanner.nextLine();
            return;
        }
        ...
        Member member = inputValidator.validationChecksUniqueId(members);

        boolean success = deleteMember(member);
        if (success) {
    ...
    public boolean deleteMember(Member member) {
        boolean removed = (members.remove(member.getMemberId()) != null);
        if (removed) {
            updateFile(MEMBERS_FILE_PATH, members);
        }
        return removed;
    }

    private void updateFile(String filePath, Map<Long, Member> members) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePath))) {
            boolean firstLine = true;
            for (Member member : members.values()) {
                String formattedRecord = member.getFirstName() + "," +
                        member.getSurname() + "," +
